...
function evaluate(tokens, expr, values) {
  ...
  for (var i = 0; i < numTokens; i++) {
    ...
    if (type === _instruction.INUMBER || type === _instruction.IVARNAME) {
      ...
      if (f.apply && f.call) {
        nstack.push(f.apply(expr, args));
      } else {
        throw new Error(f + ' is not a function');
      }
    } else if (type === _instruction.IFUNDEF) {
    ...
    } else if (type === _instruction.IEXPREVAL) {
      nstack.push(item);
    } else if (type === _instruction.IMEMBER) {
      n1 = nstack.pop();
      nstack.push(n1[item.value]);
    } else if (type === _instruction.IENDSTATEMENT) {
      nstack.pop();
    } else if (type === _instruction.IARRAY) {
      argCount = item.value;
      args = [];
...
function isExpressionEvaluator(n) {
  return n && n.type === _instruction.IEXPREVAL;
}
