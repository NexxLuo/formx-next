...
function getFooterSchema(schema) {
  if (!schema) {
    return null;
  }
  ...
  if (schema.additionalProperties) {
    ...
    let _properties = schema.properties;
    let _keys = Reflect.ownKeys(_properties);

    for (let i = 0; i < _keys.length; i++) {
      const k = _keys[i];
      const d = _properties[k];
      if (d["x-component"] === "Modal.Footer") {
        items = d.items;
        break;
      }
    }
...
const BaseModal = props => {
  ...
  const renderFooter = () => {
    let items = getFooterSchema(schema);
  ...
  const onOk = () => {
    ...
    callPrepose(
      () => {
        ...
        let _onClose = form.getFieldState(name)?.component?.[1]?.onClose;
...
export const AModal = connect(
  ...
  mapProps((props, field) => {
    ...
    let isEditor =
      field.form.getValuesIn("__DATA__.__isEditor") === true ? true : false;
    let resetProps = {
      isEditor: isEditor,
...
export const Modal = withLayoutPane(AModal, true);
