  ...
  if (index) {
    ...
    if (form) {
      ...
      if (itemState) {
        ...
        itemIsEditing = true;
        if (
          itemState.componentProps?.["x-extra-props"].isRenderSchema === true
        ) {
          itemIsSchema = true;
        }
      }
      ...
      if (!itemIsEditing || itemIsSchema) {
        ...
        if (
          listState &&
          typeof listState.fieldActions?.getItemDataIndex === "function"
        ) {
...
export function transformComponentValue(schema, value, instance) {
  ...
  if (extraProps) {
    ...
    if (extraProps.relatedKey) {
      let relatedFieldState = instance.getFieldState(extraProps.relatedKey);
...
export function isResponsiveSizeSmall(form) {
  ...
  if (form) {
    let enabledSmallLayoutSize = form.props?.context?.enabledSmallLayoutSize;
